...
Unzip it, and it will spew out a single folder, which is probably named something like `FirefightingControls-master`.
...
`C:\Program Files (x86)\Steam\steamapps\common\RimWorld\Mods\FirefightingControls-master\Assemblies`
...
`C:\Program Files (x86)\Steam\steamapps\common\RimWorld\Mods\FirefightingControls-master`
...
To do this, just click the button where it says "Branch: master", and then click the option for "beta". Congratulations! You've changed branches! Follow the same steps to download and install, except instead of `FirefightingControls-master` it will now be `FirefightingControls-beta`. You can have both versions installed, but please don't try to have both versions enabled at once using the in-game Mod menu.
